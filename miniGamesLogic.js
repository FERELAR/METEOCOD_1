...
function renderMiniStats(gameKey) {
    const container = document.querySelector(`#game-${gameKey} .stats`);
    if (container) {
        ...
        const stats = miniStats[gameKey];
        info.innerHTML = `Игр: ${stats.games} | Побед: ${stats.wins} | Лучший: ${stats.bestPoints}`;
    }
}

function renderMiniStatsForAll() {
    Object.keys(miniStats).forEach(renderMiniStats);
}

let currentCode, errors = [], selected = new Set(), attempts = 3, hintsLeft = 2, currentHint = 1, mode = 'metar', difficulty;
let stats = {score:0, level:1, wins:0, games:0};

function updateStats() {
    if (document.getElementById('score')) document.getElementById('score').textContent = stats.score;
    if (document.getElementById('level')) document.getElementById('level').textContent = stats.level;
}

function startGame(diff) {
    difficulty = diff;
    const list = gameData[mode.toUpperCase()][difficulty];  
    ...
    document.getElementById('correct-groups').style.display = 'none';
    document.getElementById('check-btn').disabled = false;
    document.getElementById('check-btn').onclick = checkGroups;
    document.getElementById('check-btn').textContent = 'Проверить';
    const codeEl = document.getElementById('meteo-code');
    codeEl.innerHTML = '';
    currentCode.split(' ').forEach((group, i) => {
...
function checkGroups() {
    const correct = selected.size === errors.length && [...selected].every(i => errors.includes(i));
    document.querySelectorAll('#meteo-code span').forEach((span, i) => {
        if (selected.has(i)) {
            span.style.background = correct ? '#27ae60' : '#e74c3c';
            span.style.color = 'white';
...
function initGameSelector() {
    document.querySelectorAll('.game-selector .btn').forEach(btn => {
        btn.addEventListener('click', function () {
            document.querySelectorAll('.game-selector .btn').forEach(b => b.classList.remove('active'));
...
function resetMiniGame() {
    // Общая функция сброса для мини-игр
    attempts = 3;
    hintsLeft = 2;
    // Другие сбросы по необходимости
}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('btn-metar').onclick = () => { mode = 'metar'; };
    document.getElementById('btn-taf').onclick = () => { mode = 'taf'; };
});