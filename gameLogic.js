...
document.addEventListener('DOMContentLoaded', function () {
    newEncodeExercise();
    updateTrainerStats();
    loadMiniStats(); // load persistent mini-game stats
...
/* -----------------------
   ...
   ----------------------- */
let miniStats = {}; // loaded from localStorage; structure like { "find-error": {games:0, wins:0, totalPoints:0, bestPoints:0, lastPoints:0}, ... }
...
function renderMiniStats(gameKey) {
    // Map known game keys to UI elements (score/level display)
    try {
        if (gameKey === 'find-error') {
            ...
            // show small summary under game if element exists
...
function resetMiniGame() {
    ...
    // Clear builder pool and dropzone
    try {
    ...
    // Clear flight planner and taf predictor
    try {
    ...
    // Clear find-error UI highlights and correct groups area
    try {
        document.querySelectorAll('#meteo-code span').forEach(span => {
            span.style.background = '';
            span.style.color = '';
            span.style.transform = '';
            span.onclick = () => { // re-enable selection handler if exists
                const idxAttr = Array.prototype.indexOf.call(span.parentNode.childNodes, span) / 2;
                // safer to reassign proper click in displayCode when startGame is called
            };
...
/* -----------------------
   Existing functions with improved resets & persistent stats
   ----------------------- */
...
function initGameSelector() {
    document.querySelectorAll('.game-selector button').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            // whenever switching games, perform a safe reset of mini-game state
            resetMiniGame();
            renderMiniStatsForAll();
        });
    });
...
function startGame(diff) {
    // Full reset of mini-games before starting a game
    resetMiniGame();
    difficulty = diff;
    attempts = 3;
    ...
    currentHint = 1;
    selected.clear();
    // Reset UI specific for find-error
    ...
    // Ensure new-task buttons hidden
    ...
    // Select code and display
    const list = gameData[mode][diff];
    const item = list[Math.floor(Math.random() * list.length)];
    currentCode = item.code;
    errors = item.errors;
    displayCode();
    // Render stats for this game
...
function displayCode() {
    ...
    words.forEach((word, i) => {
        ...
        // Wrap click binding so index is correct even in DOM
        span.onclick = () => toggleSelect(span, i);
        div.appendChild(span);
        div.appendChild(document.createTextNode(' '));
    });
    // Hide correct-groups and result area
...
function checkAnswer() {
    // If check button is disabled or no code, ignore
    ...
    if (correct) {
        ...
        playSound('ding');
        showConfetti();
        // Update persistent mini-game stats
        ...
        if (attempts === 0) {
            ...
            playSound('buzz');
            // Update persistent mini-game stats (loss)
            // give 0 points on failure
    ...
    // Instead of adding tooltips to squares, display the correct answer below the code, above buttons
    if (attempts === 0 || correct) {
        ...
        if (correctGroupsEl) {
            // Build readable string listing correct groups with indexes
            const words = currentCode.split(' ');
...
function checkCodeBuilder(timeout = false) {
    ...
    if (userCode === currentBuilderCorrect) {
        ...
        document.getElementById('builder-result').innerHTML = `<span style="color:#e74c3c; font-weight:bold;">Неправильно! (Лишние группы или неправильный порядок)</span>`;
        playSound('buzz');
...
function checkQuiz() {
    ...
    if (selected) {
        if (parseInt(selected.value) === currentQuizCorrect) {
            ...
            // update mini stats - award 10 points
            updateMiniStats('quiz-bowl', true, 10);
        } else { document.getElementById('quiz-result').innerHTML = '<span style="color:#e74c3c">Ошибка!</span>'; playSound('buzz'); updateMiniStats('quiz-bowl', false, 0); }
        setTimeout(() => {
            quizProgress++;
...
function checkFlightPlanner() {
    ...
    } else { document.getElementById('planner-result').textContent = 'Неправильно!'; playSound('buzz'); updateMiniStats('flight-planner', false, 0); }
}
